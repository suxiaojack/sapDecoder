...
FilterImpl::FilterImpl(int filter_len)
...
void FilterImpl::ForwardTransform(const vector<float>& time_signal,
...
void FilterImpl::InverseTransform(const vector<float>& freq_signal,
...
void FilterImpl::InverseFFTScaling(vector<float>* signal) const {
  assert(signal);
...
void FilterImpl::SetTimeDomainKernel(const vector<float>& kernel) {
  assert(
...
void FilterImpl::AddTimeDomainKernel(const vector<float>& kernel) {
...
void FilterImpl::SetFreqDomain(const vector<float>& kernel) {
...
void FilterImpl::AddFreqDomainKernel(const vector<float>& kernel) {
  vector<kiss_fft_cpx> temp_freq_domain_buffer(fft_len_ / 2 + 1);
...
void FilterImpl::VectorCopyWithZeroPadding(
...
void FilterImpl::AddSignalBlock(const vector<float>& signal_block) {
  assert(
...
void FilterImpl::ComplexVectorProduct(const vector<kiss_fft_cpx>& input_a,
...
void FilterImpl::GetResult(vector<float>* signal_block) {
  assert(signal_block);
